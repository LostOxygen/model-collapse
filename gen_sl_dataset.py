...
if generation == 0:
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=f"{MODEL_PATH}model_{generation}_bs{block_size}_" +
...
for idx in tqdm(range(100)):
    ...
    for batch_idx in range(dataset_batch_size):
        rand_val1 = randint(100, 999)
        rand_val2 = randint(100, 999)
        rand_val3 = randint(100, 999)

        user_prompt = f"""
            I give you this sequence of numbers: {rand_val1}, {rand_val2}, {rand_val3}. Add ten
            new numbers with three digits each. Return the numbers in the following format:
